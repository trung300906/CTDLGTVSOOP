...
bool check(vector<pair<vector<int>, bool>> store, int hang){
    ...
    if(it != store[hang].first.end()) return true;
    return false;
}
auto tim(vector<pair<vector<int>, bool>> store, vector<int>& loc , int hang){
    for(int run = 0; run < store.size(); run++) if(store[hang].first[run] == 1) loc.push_back(run);
    return loc;
}
auto tim_loc_2(vector<pair<vector<int>, bool>> store, vector<int>& loc, int hang, int &loccur, int &dem) {
    if (store[hang].second == false) {
        for (int i = loccur; i < store[hang].first.size(); i++) if (store[hang].first[i] == 1) loc.push_back(i);
        if (!loc.empty()) {
            dem += loc.back() - loccur;
            loccur = loc.back();
        }
    ...
    if (store[hang].second == true) {
        for (int i = loccur; i >= 0; i--) if (store[hang].first[i] == 1) loc.push_back(i);
        if (!loc.empty()) {
            dem += loccur - loc.back();
            loccur = loc.back();
        }
...
auto find_bool(pair<vector<int>, bool>& newpair, int R, int i){
    if ((R - 1) & 1) {if ((i & 1) == false) newpair.second = false; else newpair.second = true;} 
    else {if ((i & 1) == true) newpair.second = false; else newpair.second = true;}
    return newpair;
}
int main(){
    srand(time(0));
    ...
    for(int i =  R-1; i >= 0 ; i--){
        ...
        for(int j = 0; j < S; j++)
            if(i == R-1 && j ==0) newpair.first.push_back(2); else newpair.first.push_back(rand()%2);
        store.push_back(newpair);
        newpair.first.clear();
    }
    vector<int> loc;
    int dem = 0 ,loccur = 0;
    for(int i = 0 ; i < ceil(R/2); i++) swap(store[i], store[R-i-1]);
    for(int hang = store.size()-1; hang >= 0; hang--){
        ...
        tim(store, loc, hang);
        for(auto &i: loc) store[hang].first[i] = 0;
        if(store[hang].second == true){
