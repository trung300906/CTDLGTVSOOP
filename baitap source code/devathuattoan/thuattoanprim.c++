...
{
    vscode if (a == 0) vscode return false; // loại trừ số 0
    if (b == 0)
        return true; // loại trừ số 0
    return a < b;
}
// dijkstravscode
int main
...
vscode
    ...
    // hàm cần dùng:
    bool
    duyet(vector<pair<vector<int>, bool>> v)
{
    for (int i = 0; i < v.size(); i++)
        if (v[i].second == false)
            return false;
...
if (v[positon].second == false)
    cost += v[positon].first[way];
v[way].second = true;
v[way].first[positon] = 0;
