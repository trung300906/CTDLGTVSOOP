...
def compute_submatrix_inner_product(vector_a, vector_b):
    return void # eddit after
def process_submatrix(inputfiles, starthang,sohang, cot):
    submatrix = read_matrix(inputfiles, starthang, sohang, cot)
...
def compute_cosinematrix(numworkers, hang, cot, inputfile):
    ...
    subhang = math.ceil(subcot / numworkers)
    if subcot < 2: subcot = 2
    return_sub = parallel_multiprocess_compute(numworkers, subhang, subcot, inputfiles)
    return return_sub
