...
def compute_cosine_matrix(features, num_workers):
    # Transpose the feature matrix to calculate column-wise similarities
    features = np.array(features, dtype=np.float32).T
    num_features = features.shape[0]

    # Calculate norms for each column vector
    ...
    with Pool(processes=num_workers) as pool:
        results = pool.starmap(cosine_similarity_task, [(i, features, norms) for i in range(num_features)])
