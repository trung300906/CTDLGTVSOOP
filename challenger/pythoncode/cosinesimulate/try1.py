...
def compute_cosine_matrix(features, num_workers):
    ...
    tasks = [(i, j) for i in range(num_features) for j in range(i, num_features)]

    # Khởi tạo ma trận cosine similarity
    cosine_matrix = np.zeros((num_features, num_features), dtype=np.float32)

    # Sử dụng joblib để tính toán các cặp (i, j) song song
    results = Parallel(n_jobs=num_workers)(delayed(cosine_similarity)(i, j, features, norms) for i, j in tasks)
